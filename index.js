...
function update() {
    ...
    cin = c.in('Ready To Start? (y,n) ').then(e=>{
      if(e.toLowerCase()[0] == 'y'){
        beginGame();
      }
    }).catch(e=>{});
}
...
function beginGame() {
    ...
    console.log(`Game Started: ${tiles.length} remaining!`);
}
...
function peel(user) {
    ...
    console.log(`${user.name} Peels! ${tiles.length} remaining!`);
}

function countTiles(user) {
    if (tiles.length < peopleInGame.length) {
        console.log(`Game Over! Winner is ${user.name}`)
        for (let person of peopleInGame) {
            started = false;
...
io.on('connection', socket => {
    ...
    socket.on('trade', tile => {
        ...
        tiles.push(tile);
        console.log(`${c.name} traded! ${tiles.length} remaining.`);
    });
    socket.on('peel', e => {
        peel(c);
    });
